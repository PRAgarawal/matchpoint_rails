class UsersController < RestfulController
  ...
  def most_active
    ...
    most_active = User.where(is_dfw: User.current_user.is_dfw)
                      .order('score DESC')
                      .limit(MAX_ACTIVE)
  ...
  def render_records(friends)
    render json: friends, only: [:id, :first_name, :last_name, :skill, :score]
  end

  def render_record(user)
    render json: user, only: [:id, :first_name, :last_name, :skill, :is_dfw],
           methods: [:friend_status],
           include: {friends: {only: [:id, :first_name, :last_name, :skill]}}
  end
end
